...
async def list_rooms(
    ...
    """List public rooms."""
    items, total = await social_service.list_rooms(
        db, target_language_id, pagination["offset"], pagination["limit"]
    )
...
async def list_messages(
    room_id: UUID, db: AsyncSession = Depends(get_db), pagination: dict = Depends(get_pagination)
):
    """List room messages."""
    items, total = await social_service.list_messages(
        db, room_id, pagination["offset"], pagination["limit"]
    )
    return PaginatedResponse(
