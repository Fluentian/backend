...
async def get_admin_dashboard_stats(db: AsyncSession = Depends(get_db)):
    ...
    total_completions = (
        await db.execute(
            select(func.count(UserLessonProgress.id)).where(UserLessonProgress.completed.is_(True))
        )
    ).scalar() or 0
    avg_score = (
        await db.execute(
            select(func.avg(UserLessonProgress.mastery_score)).where(
    ...
    has_1_lesson = (
        await db.execute(
            select(func.count(func.distinct(UserLessonProgress.user_id))).where(
    ...
    dropoff_query = (
        select(
            ...
            func.count(UserLessonProgress.id).label("started"),
            (
                cast(func.count(case((UserLessonProgress.completed == True, 1))), Float)
    ...
    return {
        ...
        "timeline": {
            ...
            "activity": [
                {"date": str(a.date), "started": a.started, "completed": a.completed}
                for a in activity
            ],
        },
        "funnel": funnel,
