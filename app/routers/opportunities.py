...
async def list_opportunities(
    ...
    if type:
        query = query.where(OpportunityBoard.type == type)

    total = (
        await db.execute(
            select(func.count())
            .select_from(OpportunityBoard)
            .where(OpportunityBoard.is_active.is_(True))
        )
    ).scalar() or 0
    items = list(
...
@router.post(
    "/", response_model=OpportunityResponse, dependencies=[Depends(require_role(AppRole.admin))]
)
...
async def apply_for_opportunity(
    ...
    application = OpportunityApplication(
        opportunity_id=opportunity_id, user_id=user.id, **req.model_dump()
    )
    db.add(application)
    await db.commit()
...
@router.get(
    ...
    response_model=list[OpportunityApplicationResponse],
...
@router.patch(
    ...
    response_model=OpportunityApplicationResponse,
    dependencies=[Depends(require_role(AppRole.admin))],
)
async def update_application_status(
    application_id: UUID, status: str, db: AsyncSession = Depends(get_db)
):
