...
async def import_curriculum_csv(db: AsyncSession, csv_file: io.StringIO):
    ...
    if not default_lang:
        raise HTTPException(
            status_code=400, detail="No languages found in database. Seed languages first."
        )
    ...
    for row in [r for r in rows if r["level"] == "1"]:
        ...
        if not parent_id:
            import_results["errors"].append(
                f"Parent course {row['parent_code']} not found for unit {row['code']}"
            )
            continue
        try:
            unit = PathUnit(
    ...
    for row in [r for r in rows if r["level"] == "2"]:
        ...
        if not unit_id:
            import_results["errors"].append(
                f"Parent unit {row['parent_code']} not found for lesson {row['code']}"
            )
            continue
    ...
    for row in [r for r in rows if r["level"] in ["3", "4"]]:
        ...
        if not lesson_id:
            import_results["errors"].append(
                f"Parent lesson {row['parent_code']} not found for {row['type']} {row['code']}"
            )
            continue
        ...
        try:
            ...
            import_results["errors"].append(
                f"Error creating {row['type']} {row.get('code')}: {str(e)}"
            )
